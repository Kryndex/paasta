...
def get_spark_configuration(
    ...
    if check_if_port_free(DEFAULT_SPARK_UI_PORT):
        spark_ui_port = DEFAULT_SPARK_UI_PORT
    else:
        paasta_print(
            PaastaColors.yellow("The default Spark UI port is already in use!\n"),
            file=sys.stderr,
        )
...
def configure_and_run_docker_container(
    ...
    container_name = 'paasta_spark_run_%s_%s' % (get_username(), randint(1, 999999))
